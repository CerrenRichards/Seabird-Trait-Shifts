...
Second, we create a 1° resolution global presence-absence matrix based on the seabird distribution polygons.
...
# devtools::install_version("rgdal", version = "1.6-6", repos = "http://cran.us.r-project.org")
library(rgdal)

# devtools::install_version("maptools", version = "1.1-8", repos = "http://cran.us.r-project.org")
library(maptools)

# devtools::install_version("letsR", version = "4.0", repos = "http://cran.us.r-project.org")
library(letsR)
...
# sb <- readRDS("BOTW_seabirds_361.rds") # note this file has different binomials from the NEW_TRAITS file
# 
# # fix sb
# 
# # identify geometry column
# sb <- dplyr::rename(sb, geometry = Shape)
# 
# # ensure multipolygons to prevent errors with multisurface geometries
# sb <- ensure_multipolygons(sb)
# 
# # disable s2 processing
# sf::sf_use_s2(FALSE)
# 
# # then try to make the sf object valid
# sb <- st_make_valid(sb)
# 
# saveRDS(sb, "fixed_sb.rds")

sb <- readRDS("fixed_sb.rds")
...
seabirds <- dplyr::rename(seabirds, sciname = sci_name)

sea_spat <- as(seabirds, "Spatial")

saveRDS(sea_spat, "seabirds_spatial.rds")

# Create PAM. This can take ~30-40 minutes for 361 species
# Returns latitude and longitude with PAM for each species 
# Lat long values returned are the 1 degree cell's centroid
PAM <- letsR::lets.presab(sea_spat, resol = 1, xmn = -180, xmx = 180, ymn = -90, ymx = 90, count = TRUE)
...
PAM_crop <- letsR::lets.pamcrop(PAM, wrld_simpl, remove.sp = TRUE)
...
PAM_ocean <-  dplyr::anti_join(PAM_df, PAM_crop_df, by=c("Longitude(x)", "Latitude(y)"))
...
Run this with all the species present (n = 361). 
...
# this is important for calculating the CWM because the orders of the species
# must match the traits order
PAM_overlap <- PAM_overlap[names(PAM)]
...
# Save
saveRDS(CWM_shift, "CWM_shift.rds")
```
...
forage_overlap <- cbind(dplyr::select(CWM_all, Lat, Long),
...
## Join to the CWM_shift dataframe
# Select columns
forage_diff <- forage_diff %>% select(Lat, Long, 
                                      surface_diff, diver_diff, ground_diff, general_diff)
#Join
CWM_shift <- left_join(CWM_shift, forage_diff)
```
...
diet_overlap <- cbind(dplyr::select(CWM_all, Lat, Long),
...
## Join to the CWM_shift dataframe
# Select columns
diet_diff <- diet_diff %>% select(Lat, Long, 
                                      vert_diff, invert_diff, omnivore_diff)
#Join
CWM_shift <- left_join(CWM_shift, diet_diff)
...
### *Plot the shift in CONTINUOUS and CATEGORICAL traits*

```{r}
library(ggplot2);library(ggdist); library(tidyverse)

## Calculate the sd to summarise the shifts
stdev <- CWM_shift %>% group_by(ocean) %>% summarise(`Generation Length` = sd(GL_diff, na.rm = T),
                                                     `Clutch Size` = sd(clutch_diff, na.rm = T),
                                                     `Body Mass` = sd(bm_diff, na.rm = T),
                                                     `Surface Forager`= sd(surface_diff, na.rm = T),
                                                     `Diving Forager`= sd(diver_diff, na.rm = T),
                                                     `Generalist Forager`= sd(general_diff, na.rm = T),
                                                     `Ground Forager`= sd(ground_diff, na.rm = T),
                                                     `Vertebrate & Scavenger Diet`= sd(vert_diff, na.rm = T),
                                                     `Invertebrate Diet`= sd(invert_diff, na.rm = T),
                                                     `Omnivore Diet`= sd(omnivore_diff, na.rm = T))

## Calculate the means to summarise the shifts
means <- CWM_shift %>% group_by(ocean) %>% summarise(`Generation Length` = mean(GL_diff, na.rm = T),
                                                     `Clutch Size` = mean(clutch_diff, na.rm = T),
                                                     `Body Mass` = mean(bm_diff, na.rm = T),
                                                     `Surface Forager`= mean(surface_diff, na.rm = T),
                                                     `Diving Forager`= mean(diver_diff, na.rm = T),
                                                     `Generalist Forager`= mean(general_diff, na.rm = T),
                                                     `Ground Forager`= mean(ground_diff, na.rm = T),
                                                     `Vertebrate & Scavenger Diet`= mean(vert_diff, na.rm = T),
                                                     `Invertebrate Diet`= mean(invert_diff, na.rm = T),
                                                     `Omnivore Diet`= mean(omnivore_diff, na.rm = T))

# Gather
means2 <- means %>% gather(value = "Mean", key = "Trait", `Generation Length`:`Omnivore Diet`)
stdev2 <- stdev %>% gather(value = "SD", key = "Trait", `Generation Length`:`Omnivore Diet`)

## Join together
means <- left_join(means2, stdev2)

## For the SD plotting in ggplot, if the means are positive, need to make negative
pos <- means %>% filter(Mean > 0)
pos$SD <- pos$SD*-1
neg <- means %>% filter(Mean < 0)
means <- rbind(neg, pos)

## Assign colour category
means$colour <- ifelse(means$Mean < 0, "negative","positive")

## Assign Levels to the ocean data to order the plot
means$ocean = factor(means$ocean, levels=c("Arctic Ocean", "North Atlantic Ocean",
                                           "South Atlantic Ocean", "North Pacific Ocean",
                                           "South Pacific Ocean", "Baltic Sea",
                                           "Mediterranean Region", 
                                           "South China and Easter Archipelagic Seas",
                                           "Indian Ocean", "Southern Ocean"))

## Rename "South China and Easter Archipelagic Seas", 
means$ocean <- recode(means$ocean, `South China and Easter Archipelagic Seas` = "South China Sea")

## Assign Levels to the trait data to order the plot
means$Trait = factor(means$Trait, levels=c("Body Mass", "Generation Length",
                                           "Clutch Size", "Vertebrate & Scavenger Diet",
                                           "Surface Forager","Diving Forager",
                                           "Generalist Forager","Ground Forager",
                                           "Invertebrate Diet","Omnivore Diet"))
...
plottheme <-  theme_bw(base_size = 15)+
  ...
  theme(strip.text.x = element_text(size = 15, color = "white", face = "bold"), 
          strip.background = element_rect(fill="black"))+
  theme(legend.position = "none")

# plot
# Will have to reorder the plots in post
ggplot(data = means, aes(x = ocean, y = Mean)) +
  scale_x_discrete(limits=rev)+
  geom_bar(stat="identity", color="black", aes(fill = colour), width = 0.5)+
  geom_errorbar(aes(x=ocean, ymin = Mean, ymax=Mean-SD), width=0.4, alpha=0.9) +
    geom_hline(yintercept = 0,  size = 0.4)+
   scale_fill_manual(values=c(positive="#FF8E00",negative="#003F7D"))+
   coord_flip() +
  facet_wrap(~Trait, scales = "free_x", ncol = 4, nrow = 3)+ 
  plottheme+
  labs(x = "", y = "Mean Deviation in CWM (%)")

# Save
ggsave("allShifts.png", dpi = 300, width = 15, height = 10)
```

## One-Sample Wilcoxon Signed Rank Test 

To test whether the shifts in community weighted mean are significantly different from zero in each marine region.

```{r}

library(rstatix)

## Gather the shift data so that it can be run through a pipe
test <- CWM_shift %>% select(ocean, GL_diff, clutch_diff, 
                             bm_diff, surface_diff,diver_diff,
                             vert_diff,ground_diff, general_diff,
                             invert_diff, omnivore_diff) %>% 
                          group_by(ocean) %>%
                          gather(value = "Value", key = "Trait", 
                                 GL_diff, clutch_diff, bm_diff, 
                                 surface_diff, diver_diff,vert_diff,
                                 ground_diff, general_diff, invert_diff, 
                                 omnivore_diff)


## For each ocean region and trait, test whether signif difference from zero
res <- test %>% group_by(ocean, Trait) %>% 
       wilcox_test(Value~1, data=., mu = 0,  alternative = "two.sided") 


## Assign significance category to see easily
res$sig <- ifelse(res$p < 0.05, "***","non-signif")
...
ggplot(data = CWM_shift, aes(x = GL_diff, y = ocean)) +
  stat_halfeye()

ggplot(data = CWM_shift, aes(x = ocean, y = GL_diff)) +
  geom_violin(position = position_nudge(x = 0.2, y = 0), alpha = 0.8) +
  coord_flip() +
   scale_y_continuous(limits = c(-21, 5.6))

ggplot(data = CWM_shift, aes(x = ocean, y = clutch_diff)) +
  geom_violin( alpha = 0.8) +
  coord_flip() +
  theme_bw()+
  scale_y_continuous(limits = c(-25, 55))

ggplot(data = CWM_shift, aes(x = ocean, y = bm_diff)) +
  geom_violin(position = position_nudge(x = 0.2, y = 0), alpha = 0.8) +
  coord_flip() +
...
## CWM global map - EXTREMES REMOVED
```{r}
...
# FULL ORIGIONAL DATA SCALE

### CONTINUOUS ###

# Clutch Size

# plot a histogram and the quantiles of body mass to identify extreme outliers
hist(CWM_shift$clutch)
quantile(CWM_shift$clutch, c(.001, .999))
quantile(CWM_shift$clutch_overlap, c(.001, .999), na.rm = T)

ggplot() +
  theme_bw(base_size = 18)+
  theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank()) +
  geom_point(aes(x = Long, y = Lat, colour = clutch), data = CWM_shift) + 
  scale_color_viridis(option = "D", limits = c(1, 4.1),
  guide = guide_colorbar (barheight = unit(4, units = "mm"), 
...
# plot a histogram and the quantiles of body mass to identify extreme outliers
hist(CWM_shift$body_mass_median)
quantile(CWM_shift$body_mass_median, c(.001, .999))
quantile(CWM_shift$bm_overlap, c(.001, .999), na.rm = T)

ggplot() +
    theme_bw(base_size = 18)+
  theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank()) +
  geom_point(aes(x = Long, y = Lat, colour = body_mass_median), data = CWM_shift) + 
  scale_color_viridis(option = "D", limits = c(1.7, 3.18),
  guide = guide_colorbar (barheight = unit(4, units = "mm"), 
                          barwidth = unit(70, units = "mm"),
                          label.theme = element_text(angle = 270))) +
  scale_y_continuous(expand = c(0,0), limits = c(-84, 85))+
...
ggsave("extremes_BodyMass.png")

# Generation Length

# plot a histogram and the quantiles of Generation length to identify extreme outliers
hist(CWM_shift$GL)
quantile(CWM_shift$GL, c(.001, .999))
quantile(CWM_shift$GL_overlap, c(.001, .999), na.rm = T)

ggplot() +
     theme_bw(base_size = 18)+
  theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank()) +
  geom_point(aes(x = Long, y = Lat, colour = GL), data = CWM_shift) + 
  scale_color_viridis(option = "D",limits = c(0.95, 1.34),
  guide = guide_colorbar (barheight = unit(4, units = "mm"), 
                          barwidth = unit(70, units = "mm"),
                          label.theme = element_text(angle = 270))) +
  scale_y_continuous(expand = c(0,0), limits = c(-84, 85))+
...
ggsave("extreme_GL.png")


## shift in CWM

# Clutch Size (-BYCATCH SP)
ggplot() +
    theme_bw(base_size = 18)+
  theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank()) +
  geom_point(aes(x = Long, y = Lat, colour = clutch_overlap), data = CWM_shift) + 
  scale_color_viridis(option = "D",  limits = c(1, 4.1),
  guide = guide_colorbar (barheight = unit(4, units = "mm"), 
                          barwidth = unit(70, units = "mm"),
                          label.theme = element_text(angle = 270))) +
 ...
 ggsave("extreme_clutch_overlap.png")



# Body Mass (-BYCATCH SP)
ggplot() +   
    theme_bw(base_size = 18)+
  theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank()) +
  geom_point(aes(x = Long, y = Lat, colour = bm_overlap), data = CWM_shift) + 
  scale_color_viridis(option = "D", limits = c(1.7, 3.18), 
  guide = guide_colorbar (barheight = unit(4, units = "mm"), 
                          barwidth = unit(70, units = "mm"),
                          label.theme = element_text(angle = 270))) +
 ...
 ggsave("extremes_bodymass_overlap.png")
 
 

# Generation Length (-BYCATCH SP) 
 ggplot() +
    theme_bw(base_size = 18)+
  theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank()) +
  geom_point(aes(x = Long, y = Lat, colour = GL_overlap), data = CWM_shift) + 
  scale_color_viridis(option = "D", limits = c(0.95, 1.34), 
  guide = guide_colorbar (barheight = unit(4, units = "mm"), 
                          barwidth = unit(70, units = "mm"),
                          label.theme = element_text(angle = 270))) +
...
ggplot() +
     theme_bw(base_size = 18)+
  theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank()) +
  geom_point(aes(x = Long, y = Lat, colour = foraging_guild), data = CWM_shift) + 
    guides(colour = guide_legend(override.aes = list(size=5)))+
  scale_color_manual(values = c( "#481567FF","#95D840FF","#20A387FF" , "#33638DFF"))+ 
  scale_y_continuous(expand = c(0,0), limits = c(-84, 85))+
  scale_x_continuous(expand = c(0,0), limits = c(-180, 180))+
...
# Foraging Guild - BYCATCH
ggplot() +
     theme_bw(base_size = 18)+
  theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank()) +
  geom_point(aes(x = Long, y = Lat, colour = foraging_overlap), data = CWM_shift) + 
  guides(colour = guide_legend(override.aes = list(size=5)))+
  scale_color_manual(values = c( "#481567FF","#95D840FF","#20A387FF" , "#33638DFF"))+ 
  scale_y_continuous(expand = c(0,0), limits = c(-84, 85))+
  scale_x_continuous(expand = c(0,0), limits = c(-180, 180))+
...
ggplot() +
     theme_bw(base_size = 18)+
  theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank()) +
  geom_point(aes(x = Long, y = Lat, colour = diet_5cat), data = CWM_shift) + 
    guides(colour = guide_legend(override.aes = list(size=5)))+
  scale_color_manual(values = c("#33638DFF","#20A387FF","#95D840FF"))+ 
  scale_y_continuous(expand = c(0,0), limits = c(-84, 85))+
  scale_x_continuous(expand = c(0,0), limits = c(-180, 180))+
  theme(legend.position = "bottom")+
  theme(axis.title.x=element_blank(), axis.title.y=element_blank(), legend.title = element_blank())+
...
ggplot() +
     theme_bw(base_size = 18)+
  theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank()) +
  geom_point(aes(x = Long, y = Lat, colour = diet_5cat_overlap), data = CWM_shift) + 
  guides(colour = guide_legend(override.aes = list(size=5)))+
  scale_color_manual(values = c("#33638DFF","#20A387FF","#95D840FF"))+ 
  scale_y_continuous(expand = c(0,0), limits = c(-84, 85))+
  scale_x_continuous(expand = c(0,0), limits = c(-180, 180))+
...
ggplot() +
  ...
  theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank()) +
  geom_point(aes(x = Long, y = Lat, colour = clutch), data = CWM_shift) + 
  scale_color_viridis(option = "D", limits = c(1, 9.4),
...
ggplot() +
  ...
  theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank()) +
  geom_point(aes(x = Long, y = Lat, colour = body_mass_median), data = CWM_shift) + 
  scale_color_viridis(option = "D", limits = c(1.4, 4.5),
  guide = guide_colorbar(title.position = 'top', title.hjust = 0.5, label.hjust = 0.5,
  barheight = unit(3, units = "mm"), barwidth = unit(100, units = "mm"))) +
  scale_y_continuous(expand = c(0,0), limits = c(-84, 85))+
  scale_x_continuous(expand = c(0,0), limits = c(-180, 180))+
...
ggplot() +
  ...
  theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank()) +
  geom_point(aes(x = Long, y = Lat, colour = GL), data = CWM_shift) + 
  scale_color_viridis(option = "D",limits = c(0.84, 1.49),
  guide = guide_colorbar (title.position = 'top', title.hjust = 0.5, label.hjust = 0.5,
  barheight = unit(3, units = "mm"), barwidth = unit(100, units = "mm"))) +
  scale_y_continuous(expand = c(0,0), limits = c(-84, 85))+
  scale_x_continuous(expand = c(0,0), limits = c(-180, 180))+
  theme(legend.position = "bottom", legend.box = "vertical")+
  labs(colour = "Generation Length") + 
  theme(axis.title.x=element_blank(), axis.title.y=element_blank())+
    geom_sf(data = world, fill = "grey60", colour = "grey20", size = 0.25)
...
ggplot() +
  ...
  scale_x_continuous(expand = c(0,0), limits = c(-180, 180))+
  theme(legend.position = "bottom", legend.box = "vertical")+
  labs(colour = "Clutch Size - Bycatch Extinction") + 
  theme(axis.title.x=element_blank(), axis.title.y=element_blank())+
    geom_sf(data = world, fill = "grey60", colour = "grey20", size = 0.25)

 ggsave("clutch_overlap.png")
...
ggplot() +
  ...
  scale_x_continuous(expand = c(0,0), limits = c(-180, 180))+
  theme(legend.position = "bottom", legend.box = "vertical")+
  labs(colour = "Body Mass - Bycatch Extinction") + 
  theme(axis.title.x=element_blank(), axis.title.y=element_blank())+
    geom_sf(data = world, fill = "grey60", colour = "grey20", size = 0.25)

 ggsave("bodymass_overlap.png")
...
# Generation Length (-BYCATCH SP) 
 ggplot() +
  ...
  scale_x_continuous(expand = c(0,0), limits = c(-180, 180))+
  theme(legend.position = "bottom", legend.box = "vertical")+
  labs(colour = "Generation Length - Bycatch Extinction") + 
  theme(axis.title.x=element_blank(), axis.title.y=element_blank())+
    geom_sf(data = world, fill = "grey60", colour = "grey20", size = 0.25)

 ggsave("GL_overlap.png")
