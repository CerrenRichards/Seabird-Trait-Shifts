...
knitr::opts_knit$set(root.dir = '/Users/cerridwenr/Desktop/Bycatch Trait Shifts')
...
birds$all <- apply(birds[,3:343],1,max) 
...
## This is achieved through subtracting the vessel column (137) from all seabird columns (3:136)
for(i in colnames(bycatch_birds[,3:136])) {
  bycatch_birds[i] <- bycatch_birds[i] - bycatch_birds[137]
}

## Ensure all the values are rounded to 0 decimal places
birdround <- round(bycatch_birds[3:136], digits = 0)
...
full$all <- apply(full[,3:136],1,max) 
removed$all <- apply(removed[,3:136],1,max)
