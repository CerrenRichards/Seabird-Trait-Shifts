...
stdev <- CWM_shift %>% group_by(ocean) %>% summarise(`Generation Length` = sd(GL_diff, na.rm = T),
                                                     ...
                                                     `Generalist Forager`= sd(vert_diff, na.rm = T),
                                                     `Ground Forager`= sd(ground_diff, na.rm = T),
                                                     `Vertebrate & Scavenger Diet`= sd(diver_diff, na.rm = T),
...
means <- CWM_shift %>% group_by(ocean) %>% summarise(`Generation Length` = mean(GL_diff, na.rm = T),
                                                     ...
                                                     `Vertebrate & Scavenger Diet`= mean(diver_diff, na.rm = T),
...
res <- wilcox.test(test$ground_diff, mu = 0, alternative = "two.sided")

test %>% wilcox_test(ground_diff ~ 1, mu = 0, alternative = "two.sided") 
wilcox.test(test$ground_diff, mu = 0, alternative = "two.sided")


test <- CWM_shift %>% filter(ocean == "Indian Ocean") %>% select(ground_diff)
