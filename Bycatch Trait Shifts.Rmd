...
shift_coord <- CWM_shift %>% select(Lat, Long)
...
nearest_polygon <- nearest_polygon %>% rename(Bioregion = NAME_E) %>% select(Colony.Id, Bioregion)
nearest_polygon$geometry <- NULL


# Recreate the colonly dataframe for plotting and join the bioregion data
colonies <- sb.populations %>% group_by(Colony.Id) %>% 
              summarise(Lat = min(Lat),
                        Lon = min(Lon)) %>%
              left_join(., nearest_polygon)


## Replace the NAs in the Location column with the Colony column cells
#colonies <- colonies %>% mutate(Bioregion = coalesce(Bioregion, Colony))
